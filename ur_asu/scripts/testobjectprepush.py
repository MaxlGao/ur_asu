...
from ur_asu.custom_libraries.actionlibrariesmax import move, append_new_act
...
class OverController(Node):
    def __init__(self, **kwargs):
        ...
        start_position = [0.00, -0.550, GRIPPER_TABLE[0] + self.vert_offset]
    ...
    # def parse_acts(self):
    #     goals = {}
    #     for traj_name, points in self.acts.items():
    ...
    def check_for_new_pose(self):
        ...
        if self.executing:
            # We're running something currently; check back later
            return
    ...
    def update_act(self):
        ...
        moves = self.gripper.pointspan_to_acts_safe([self.ee_position, self.ee_euler], 
    ...
    #     # self.get_logger().info(f"✔ act {traj_name} completed with status: {self.status_to_str(status)}")
    ...
    #         self.execute_next_act()
    #     else:
    #         raise RuntimeError("act failed: " + str(result.error_string))
